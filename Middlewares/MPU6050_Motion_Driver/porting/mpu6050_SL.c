...
uint8_t MPU6050_config(void) {
  ...
  /*
   * Known Bug -
   * DMP when enabled will sample sensor data at 200Hz and output to FIFO at the
   * rate specified in the dmp_set_fifo_rate API. The DMP will then sent an
   * interrupt once a sample has been put into the FIFO. Therefore if the
   * dmp_set_fifo_rate is at 25Hz there will be a 25Hz interrupt from the MPU
   * device.
   *
...
dmp_enable_feature( DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|
  ...
  short gyro[3];
  for (int i = 0; i<10; i++) {
    if(0 == mpu_get_gyro_reg(gyro,(inv_time_t *)&timestamp)){
      sum_bias[0] += gyro[0]/16.4;
      sum_bias[1] += gyro[1]/16.4;
      sum_bias[2] += gyro[2]/16.4;
    };
  }
  gyro_bias[0] = sum_bias[0]/10;
  gyro_bias[1] = sum_bias[1]/10;
  gyro_bias[2] = sum_bias[2]/10;
  hal.dmp_on = 1;
  return 1;
}
...
void setup_gyro(void) {
  ...
  // MPU_DEBUG_FUNC();
  if (hal.sensors & ACCEL_ON) mask |= INV_XYZ_ACCEL;
...
uint8_t MPU6050_get_sensor(float *Acceleration, float *Angular_velocity,
                           float *Quaternion, float *Euler) {
...
void send_status_compass() {
  ...
  MPL_LOGI("Compass: %7.4f %7.4f %7.4f ", data[0] / 65536.f, data[1] / 65536.f,
           data[2] / 65536.f);
  MPL_LOGI("Accuracy= %d\r\n", accuracy);
}
#endif
...
void run_self_test(void) {
  ...
  if (result == 0x7) {
    ...
    MPL_LOGI("gyro: %7.4f %7.4f %7.4f\n", gyro[0] / 65536.f, gyro[1] / 65536.f,
             gyro[2] / 65536.f);
    /* Test passed. We can trust the gyro data here, so now we need to update
     * calibrated data*/

#ifdef USE_CAL_HW_REGISTERS
    /*
     * This portion of the code uses the HW offset registers that are in the
     * MPUxxxx devices instead of pushing the cal data to the MPL software
     * library
     */
    unsigned char i = 0;
    ...
    if (!(result & 0x1)) MPL_LOGE("Gyro failed.\n");
...
void handle_input(void) {
  ...
  switch (c) {
    ...
    case '8':
      ...
      setup_gyro();
      if (!(hal.sensors & ACCEL_ON)) inv_accel_was_turned_off();
      break;
    case '9':
      hal.sensors ^= GYRO_ON;
      setup_gyro();
      if (!(hal.sensors & GYRO_ON)) inv_gyro_was_turned_off();
      break;
#ifdef COMPASS_ENABLED
    case '0':
      hal.sensors ^= COMPASS_ON;
      setup_gyro();
      if (!(hal.sensors & COMPASS_ON)) inv_compass_was_turned_off();
      break;
#endif
    ...
    case 'p':
      if (hal.dmp_on) /* LP accel is not compatible with the DMP. */
        break;
    ...
    case 'f':
      ...
      if (hal.dmp_on) {
        ...
        if (hal.sensors & ACCEL_ON) mask |= INV_XYZ_ACCEL;
        if (hal.sensors & GYRO_ON) mask |= INV_XYZ_GYRO;
        if (hal.sensors & COMPASS_ON) mask |= INV_XYZ_COMPASS;
        mpu_configure_fifo(mask);
...
//     //   printf("加速度(g/s2): %d  %d  %d\n", (int)float_data[0],
//     //          (int)float_data[1], (int)float_data[2]);
//     // }
//     //
...
int MPU6050_getValue(void) {
  ...
  unsigned long timestamp;
   ...
   if (sensors & INV_WXYZ_QUAT)
    ...
    IMU_Result.EulerX = pitch;
    IMU_Result.EulerY = roll; 
     ...
     */
        ...
        // if (inv_get_sensor_type_accel(data, &accuracy, (inv_time_t *)&timestamp)) {
    //  IMU_Result.AccelX = data[0] * 1.0 / (1 << 16);
    //  IMU_Result.AccelY = data[1] * 1.0 / (1 << 16);
    //  IMU_Result.AccelZ = data[2] * 1.0 / (1 << 16);
    //   // printf("加速度(g/s2): %d  %d  %d\n", (int)IMU_Result.AccelX,
    //          // (int)IMU_Result.AccelY, (int)IMU_Result.AccelZ);
    // }
    //
    ...
    //          // (int)IMU_Result.GyroY, (int)IMU_Result.GyroZ);
    // }
    // }
  //  if(0 == mpu_get_gyro_reg(gyro,(inv_time_t *)&timestamp)){
  //    IMU_Result.GyroX = gyro[0]/16.4 - gyro_bias[0];
  //    IMU_Result.GyroY = gyro[1]/16.4 - gyro_bias[1];
  //    IMU_Result.GyroZ = gyro[2]/16.4 - gyro_bias[2];
  //   
  //    printf("\n角速度(degree/s):GyroX= %d,GyroY= %d,GyroZ= %d\n", (int)IMU_Result.GyroX,
  //            (int)IMU_Result.GyroY, (int)IMU_Result.GyroZ);
  // };
...
//MPU6050陀螺仪角速度获取函数，可以达到1khz，快得很。DMP只能200hz
void mpu6050_get_gyrodata(void)
{
   short gyro[3];
   unsigned long timestamp;
if(0 == mpu_get_gyro_reg(gyro,(inv_time_t *)&timestamp)){
